...
class AllocationByProcessController extends CI_Controller {
	...
	public function newAddProcessAllocation(){
        ...
        $compName=trim($this->input->post('currentBillCompany'));
        ...
        if(empty($checkBill)){
              ...
              $insertData=array('billNo'=>$currentBillNo,'date'=>$currentDate,'retailerName'=>$currentBillRetailer,'compName'=>$currentBillCompany,'netAmount'=>$currentBillAmount,'pendingAmt'=>$currentBillAmount);
              $this->AllocationByManagerModel->insert('bills',$insertData);
              if($this->db->affected_rows()>0){
              		$id=$this->db->insert_id();
                    $data['id']=$id;
					$data['compName']=$currentBillCompany;
					...
					$data['current']=$this->AllocationByManagerModel->load('bills',$id);
		...
		// $this->load->view("commanTransactions/addAllocationByProcessView",$data);
	}

	public function newProcessAllocation($id,$compName){
        $data['id']=$id;
...
public function getPastBills(){
	...
	if(!empty($billNo)){
		...
		if(!empty($newRouteBills)){
            ...
            <?php  }
				...
				<td><?php echo $items['netAmount']; ?></td>
				<td><?php echo $items['SRAmt']; ?></td>
				<td><?php echo $items['receivedAmt']+$items['debit']+$items['officeAdjustmentBillAmount']+$items['otherAdjustment'];?></td>
		            <!-- <td></td> -->
		            <td><?php echo $items['cd']; ?></td>
		            <td><?php echo $items['pendingAmt']; ?></td>
		           <td></td>
	...
	//bill no
				...
				if($this->db->affected_rows()>0){ 
					...
					if(!empty($billId)){
						foreach($billId as $items){
							...
							$this->AllocationByManagerModel->insertTableData('allocationsbills',$billAllocationData); 
							if($this->db->affected_rows()>0){
								$rtData=array
								(
		...
		public function SaveConfirmOld(){
											...
											if(!$rs==0){
												...
												if(!empty($checkHistory)){
													$history=array(
														'billId'=>$billId[$i]['id'],
														'allocationId' => $data['alloca'][0]['id'],
														'transactionStatus' =>'Allocated in '.$data['alloca'][0]['allocationCode'],
														'transactionDate'=>date('Y-m-d H:i:sa'),
														'transactionBy'=>trim($this->session->userdata[$this->projectSessionName]['id'])
													);
													$this->AllocationByManagerModel->update('bill_transaction_history',$history,$checkHistory[0]['id']);
